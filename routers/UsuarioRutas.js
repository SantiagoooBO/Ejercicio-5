...
ruta.post("/agregarProducto", (req, res) => {
    console.log(req.body); // Muestra el cuerpo completo del request
    console.log(req.body.nombre, req.body.descripcion, req.body.precio); // Accede a los campos específicos
...
ruta.get("/borrarProducto/:id", async (req, res) => {
    const productobd = new ProductoBD();
    await productobd.borrarProductos(req.params.id);
    res.redirect("/productos");
});
